...
export function Services() {
  ...
  const particuliers: ServiceCardData[] = t.services.index.filter((x) =>
    PARTICULIERS_KEYS.includes(x.key),
  );
  const pro: ServiceCardData[] = t.services.proItems.map((p) => ({
    key: p.key,
    name: p.title,
    desc: p.desc,
  }));
        ...
        >
          {cards.map((it, i) => (
            <ServiceCard
              ...
              card={it}
              i={i}
              tint={SERVICE_TINTS[i % SERVICE_TINTS.length]}
              href="/nos-services"
